    ...
    <div className="mx-auto max-w-4xl space-y-5 pb-10">
      ...
      {showAI && (
        <Card className="space-y-3 ring-brand-500/20">
          ...
          <p className="text-xs text-ink-500">AI only prefills the form. You review every field before a draft is created.</p>
      ...
      <button onClick={() => { localStorage.removeItem(DRAFT_KEY); setDraft(initialDraft()); setStep(0) }} className="w-full text-center text-xs text-ink-500 hover:text-ink-500">Discard saved wizard draft</button>
    </div>
  )
}
...
function BasicsStep({ draft, update }: StepProps) {
  return <div className="space-y-5">
    ...
    <div><SectionLabel>Choose a starting template</SectionLabel><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{TEMPLATES.map(([value, title, body]) => <button key={value} onClick={() => update('template', value)} className={`rounded-2xl p-4 text-left ring-1 transition ${draft.template === value ? 'bg-brand-600/15 ring-brand-500/40' : 'bg-paper-0 ring-ink-200 hover:bg-paper-100'}`}><p className="text-sm font-medium text-ink-950">{title}</p><p className="mt-1 text-xs leading-5 text-ink-500">{body}</p></button>)}</div></div>
  </div>
}
...
function ContributionsStep({ draft, update }: StepProps) {
  ...
  const needsAmount = draft.contributionMode !== 'voluntary'
  return <div className="space-y-5">
    <div><SectionLabel>Contribution model</SectionLabel><div className="grid gap-3 sm:grid-cols-2">{modes.map(([value, title, body]) => <button key={value} onClick={() => update('contributionMode', value)} className={`rounded-2xl p-4 text-left ring-1 ${draft.contributionMode === value ? 'bg-brand-600/15 ring-brand-500/40' : 'bg-paper-0 ring-ink-200'}`}><p className="text-sm font-medium text-ink-950">{title}</p><p className="mt-1 text-xs text-ink-500">{body}</p></button>)}</div></div>
    <Card className="grid gap-4 sm:grid-cols-2">
      ...
      <Field label="Grace period"><div className="flex items-center gap-2"><input type="number" min={0} max={30} className={inputClass} value={draft.graceDays} onChange={(e) => update('graceDays', Number(e.target.value))} /><span className="text-sm text-ink-500">days</span></div></Field>
      <label className="flex items-center gap-3 text-sm text-ink-700 sm:col-span-2"><input type="checkbox" className="h-4 w-4 accent-brand-500" checked={draft.memberTotalsVisible} onChange={(e) => update('memberTotalsVisible', e.target.checked)} />Show each member’s confirmed contribution total to the group</label>
    </Card>
    <Card><p className="text-xs leading-5 text-ink-500">Schedules create reminders and status tracking. Members still approve every contribution themselves; Kolektibo never performs an automatic debit.</p></Card>
  </div>
}
...
function SpendingStep({ draft, update }: StepProps) {
  ...
  return <div className="space-y-5">
    <div><div className="flex items-center justify-between"><SectionLabel>Spending categories</SectionLabel><Button variant="ghost" onClick={() => setCategories([...draft.categories, defaultCategory('')])}>+ Category</Button></div><div className="space-y-3">{draft.categories.map((category, index) => <Card key={category.id} className="space-y-4"><div className="flex items-center justify-between"><p className="text-sm font-medium text-ink-950">Category {index + 1}</p><button disabled={draft.categories.length === 1} onClick={() => setCategories(draft.categories.filter((item) => item.id !== category.id))} className="text-xs text-rose-400 disabled:opacity-30">Remove</button></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Name"><input className={inputClass} value={category.name} onChange={(e) => setCategories(draft.categories.map((item) => item.id === category.id ? { ...item, name: e.target.value } : item))} placeholder="Equipment" /></Field><Field label="Description" hint="Optional"><input className={inputClass} value={category.description} onChange={(e) => setCategories(draft.categories.map((item) => item.id === category.id ? { ...item, description: e.target.value } : item))} /></Field><Field label="Per-transaction cap" hint="Blank means no cap"><MoneyInput value={category.perSpendCap} onChange={(value) => setCategories(draft.categories.map((item) => item.id === category.id ? { ...item, perSpendCap: value } : item))} /></Field><Field label="Rolling monthly cap" hint="Enforced by contract v2"><MoneyInput value={category.monthlyCap} onChange={(value) => setCategories(draft.categories.map((item) => item.id === category.id ? { ...item, monthlyCap: value } : item))} /></Field></div><label className="flex items-center gap-2 text-xs text-ink-500"><input type="checkbox" className="accent-brand-500" checked={category.attachmentRequired} onChange={(e) => setCategories(draft.categories.map((item) => item.id === category.id ? { ...item, attachmentRequired: e.target.checked } : item))} />Require an invoice or receipt for this category</label></Card>)}</div></div>
    <Card className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Default request expiry"><div className="flex items-center gap-2"><input type="number" min={1} max={90} className={inputClass} value={draft.expirationDays} onChange={(e) => update('expirationDays', Number(e.target.value))} /><span className="text-sm text-ink-500">days</span></div></Field><label className="flex items-center gap-3 self-end pb-3 text-sm text-ink-700"><input type="checkbox" className="accent-brand-500" checked={draft.membersMayPropose} onChange={(e) => update('membersMayPropose', e.target.checked)} />Allow members to propose spending</label></div></Card>
...
function MoneyInput({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <div className="flex items-center gap-2"><span className="text-ink-500">₱</span><input inputMode="decimal" className={inputClass} value={value} onChange={(e) => onChange(e.target.value.replace(/[^\d.]/g, ''))} placeholder="0" /></div>
}

function ReviewCard({ title, rows }: { title: string; rows: [string, string][] }) {
  return <Card><SectionLabel>{title}</SectionLabel><div className="space-y-2">{rows.map(([label, value]) => <div key={`${label}-${value}`} className="flex justify-between gap-4 text-sm"><span className="text-ink-700">{label}</span><span className="text-right text-ink-500">{value}</span></div>)}</div></Card>
}
