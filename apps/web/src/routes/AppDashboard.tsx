    ...
    return (
      <div className="mx-auto max-w-2xl space-y-5 py-6">
        <Card className="space-y-4 ring-gold-500/20">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-white">The demo is ready to explore</h1>
    ...
    <div className="space-y-7 pb-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Your groups, pending setup, and money activity in one place.</p>
        </div>
        <Button onClick={() => navigate({ to: '/app/pools/new' })}>Create a pool</Button>
      </div>

      {pools.isLoading ? (
        <Card><p className="text-sm text-slate-400">Loading your workspace…</p></Card>
      ) : poolRows.length === 0 ? (
        <NoPools />
      ) : (
        <>
                  ...
                  <Card className="h-full transition hover:bg-ink-700/60">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-slate-500">{pool.currency_label} · {role}</p>
                      </div>
                      <Badge tone={pool.status === 'active' ? 'green' : 'gold'}>{pool.status}</Badge>
                    </div>
                    {pool.description && <p className="mt-3 line-clamp-2 text-sm text-slate-400">{pool.description}</p>}
                  </Card>
                </Link>
              ))}
            </div>
    ...
    <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
      <Card className="space-y-5 bg-linear-to-br from-brand-700/30 to-ink-800/60 p-6">
        <div>
          ...
          <h2 className="mt-4 text-2xl font-semibold text-white">Create your first group treasury</h2>
      ...
      <Card className="space-y-4 p-6">
        <h2 className="font-semibold text-white">Want to see the complete flow?</h2>
...
function Metric({ label, value, detail, tone = 'brand' }: { label: string; value: number; detail: string; tone?: 'brand' | 'gold' }) {
  return (
    <Card>
      ...
      <p className={`mt-2 text-3xl font-bold ${tone === 'gold' ? 'text-gold-400' : 'text-white'}`}>{value}</p>
