...
export function OAuthCallbackPage() {
  ...
  if (loading || !session || consented) return <Centered><p className="text-sm text-slate-400">Finishing secure sign-in…</p></Centered>
  return <Centered><div className="text-center"><h1 className="text-2xl font-semibold text-white">Before you continue</h1><p className="mt-2 text-sm text-slate-400">Confirm the launch eligibility and privacy terms for your Google account.</p></div><Card className="space-y-4"><label className="flex items-start gap-2 text-sm text-slate-300"><input type="checkbox" className="mt-1 accent-brand-500" checked={terms} onChange={(e) => setTerms(e.target.checked)} /><span>I agree to the <Link to="/legal/terms" className="text-brand-400">Terms</Link> and <Link to="/legal/privacy" className="text-brand-400">Privacy Notice</Link>.</span></label><label className="flex items-start gap-2 text-sm text-slate-300"><input type="checkbox" className="mt-1 accent-brand-500" checked={age} onChange={(e) => setAge(e.target.checked)} /><span>I confirm that I am at least 18 years old.</span></label><Button className="w-full" disabled={!terms || !age} loading={accept.isPending} onClick={() => accept.mutate()}>Continue</Button></Card></Centered>
}
...
export function OnboardingWalletPage() {
  ...
  return <Centered><StepHeader step="2 of 3" title="Connect your signing wallet" body="You may browse and join first, but a verified wallet is required before contributing or approving." /><Card className="space-y-4"><div className="flex items-center justify-between"><p className="font-medium text-white">Wallet status</p><Badge tone={verified ? 'green' : 'gold'}>{verified ? 'verified' : 'not connected'}</Badge></div><p className="text-sm leading-6 text-slate-400">The testnet beta creates a device-held key and proves ownership with a signed challenge. Production approvers will migrate to recovery-ready passkeys.</p><Link to="/app/wallet"><Button className="w-full">{verified ? 'Review wallet' : 'Set up wallet'}</Button></Link><Link to="/onboarding/recovery" className="block text-center text-xs text-brand-400">{verified ? 'Continue' : 'I’ll connect one later'}</Link></Card></Centered>
}

export function OnboardingRecoveryPage() {
  return <Centered><StepHeader step="3 of 3" title="Understand signer recovery" body="Approver access must survive a lost phone or browser." /><Card className="space-y-4"><Badge tone="gold">Testnet limitation</Badge><p className="text-sm leading-6 text-slate-300">Current beta wallets use a manually backed-up Stellar secret. Never share it with Kolektibo or another pool member.</p><p className="text-sm leading-6 text-slate-400">Before mainnet, approvers will be required to enroll two passkeys. Contract v2 will also let the remaining quorum rotate a lost approver.</p><Link to="/onboarding/complete"><Button className="w-full">I understand</Button></Link></Card></Centered>
}

export function OnboardingCompletePage() {
  const pendingInvite = localStorage.getItem('kolektibo.join.pending')
  return <Centered><div className="text-center"><Badge tone="green">Account ready</Badge><h1 className="mt-4 text-2xl font-semibold text-white">Welcome to Kolektibo</h1><p className="mt-2 text-sm text-slate-400">Create a private treasury, join an invitation, or explore the testnet scenario first.</p></div><div className="grid gap-3">{pendingInvite && <Link to="/invite/$code" params={{ code: pendingInvite }}><Button className="w-full">Continue invitation</Button></Link>}<Link to="/app/pools/new"><Button className="w-full">Create a pool</Button></Link><Link to="/app/pools"><Button variant="ghost" className="w-full">Join with a code</Button></Link><Link to="/demo" className="text-center text-sm text-brand-400">Explore demo</Link></div></Centered>
}
...
function StepHeader({ step, title, body }: { step: string; title: string; body: string }) { return <div className="text-center"><p className="text-xs font-medium text-brand-400">Onboarding · {step}</p><h1 className="mt-2 text-2xl font-semibold text-white">{title}</h1><p className="mt-2 text-sm text-slate-400">{body}</p></div> }
