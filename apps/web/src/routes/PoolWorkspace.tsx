...
export function SpendDetailPage() {
  ...
  return <PoolPage title={`Spend request #${spend.id}`} intro="The contract record is authoritative; descriptions and receipts are supporting metadata."><Card className="space-y-5"><div className="flex items-start justify-between"><div><p className="text-3xl font-bold text-ink-950">{peso(spend.amount)}</p><p className="mt-1 text-sm text-ink-500">{spend.category} · {spend.memo || 'No purpose supplied'}</p></div><Badge tone={spend.executed ? 'green' : ready ? 'brand' : 'gold'}>{spend.executed ? 'released' : ready ? 'ready' : 'pending'}</Badge></div><dl className="grid gap-4 text-sm sm:grid-cols-2"><Detail label="Proposed by" value={ctx.nameFor(spend.proposer)} /><Detail label="Payee" value={ctx.nameFor(spend.recipient)} /><Detail label="Payee address" value={shortAddr(spend.recipient, 8, 6)} /><Detail label="Approvals" value={`${spend.approvals.length} of ${ctx.state.data?.threshold}`} /></dl></Card><Card><SectionLabel>Approvers</SectionLabel><div className="flex flex-wrap gap-2">{ctx.state.data?.officers.map((address) => <Badge key={address} tone={spend.approvals.includes(address) ? 'green' : 'slate'}>{spend.approvals.includes(address) ? '✓' : '○'} {ctx.nameFor(address)}</Badge>)}</div></Card>{metadata.data && <Card><SectionLabel>Supporting record</SectionLabel><p className="text-sm text-ink-700">{metadata.data.note || 'No additional description.'}</p><p className="mt-2 text-xs text-ink-500">{metadata.data.receipt_urls.length} attachment{metadata.data.receipt_urls.length === 1 ? '' : 's'}</p></Card>}<p className="text-xs text-ink-500">Approval and release actions remain on the pool overview during the current testnet beta. The production transaction-review panel will move here with passkey signing.</p></PoolPage>
}
...
export function PoolMembersPage() {
  ...
  return <PoolPage title="People" intro="Directory roles are separate from the on-chain approver set."><div className="flex justify-end">{ctx.membership?.role === 'officer' && <Link to="/app/pools/$poolId/invites" params={{ poolId: ctx.poolId }}><Button>Invite people</Button></Link>}</div>{ctx.roster.data?.length ? <div className="space-y-3">{ctx.roster.data.map((member) => { const isApprover = !!member.stellar_address && ctx.state.data?.officers.includes(member.stellar_address); const contribution = member.stellar_address ? chainMembers.get(member.stellar_address)?.contributed : undefined; return <Card key={member.user_id} className="flex items-center justify-between gap-4"><div><p className="font-medium text-ink-950">{member.display_name_override ?? member.profile?.display_name ?? 'Member'}</p><p className="mt-1 text-xs text-ink-500">{member.role}{member.stellar_address ? ` · ${shortAddr(member.stellar_address)}` : ' · wallet not connected'}</p></div><div className="text-right">{isApprover && <Badge tone="gold">approver</Badge>}{contribution !== undefined && <p className="mt-2 text-xs text-ink-500">{peso(contribution)}</p>}</div></Card> })}</div> : <Loading />}</PoolPage>
}
...
export function PoolRulesPage() {
  ...
  return <PoolPage title="Rules" intro="Human-readable directory rules beside the policy currently enforced on Stellar.">{ctx.policy ? <><Card><p className="text-sm leading-6 text-ink-700">{ctx.policy.summary}</p></Card><div className="grid gap-4 md:grid-cols-2"><Card><SectionLabel>Contributions</SectionLabel><Detail label="Dues" value={ctx.policy.dues ? `${peso(ctx.policy.dues.amount)} ${ctx.policy.dues.period}` : 'Voluntary'} /><Detail label="Settlement" value={ctx.policy.currency} /></Card><Card><SectionLabel>Approvals</SectionLabel><Detail label="Current contract" value={`${ctx.state.data?.threshold ?? ctx.policy.approval.threshold} of ${ctx.state.data?.officers.length ?? ctx.policy.approval.of}`} />{tiers?.map((tier) => <Detail key={tier.minimumAmount} label={`From ${peso(tier.minimumAmount)}`} value={`${tier.requiredApprovals} approvals · contract v2`} />)}</Card></div><Card><SectionLabel>Categories</SectionLabel><div className="space-y-3">{(categories ?? ctx.policy.categories).map((category) => <div key={category.name} className="flex justify-between gap-4 text-sm"><span className="text-ink-700">{category.name}</span><span className="text-right text-ink-500">{categoryLimitLabel(category)}</span></div>)}</div></Card></> : <Empty message="No display policy is stored for this pool." />}</PoolPage>
}
...
export function PoolReportsPage() {
  ...
  return <PoolPage title="Reports" intro="Export contract-backed contribution and spending records for group review."><Card className="space-y-4"><div><h2 className="font-semibold text-ink-950">Audit CSV</h2><p className="mt-1 text-sm text-ink-500">Includes member contribution totals and every current spend request. Transaction-level dates and hashes will come from the production indexer.</p></div><Button disabled={!ctx.state.data} onClick={downloadCsv}>Download CSV</Button></Card><Card><p className="text-sm text-ink-700">PDF audit packs, receipt bundles, and date-range filtering are scheduled after the event indexer and attachment storage are enabled.</p></Card></PoolPage>
}
...
export function PoolSettingsPage({ section }: { section: 'general' | 'contributions' | 'governance' | 'security' | 'archive' }) {
  ...
  return <PoolPage title={content[0]} intro={content[1]}><Card className="space-y-4"><Badge tone={ctx.pool.data?.status === 'active' ? 'green' : 'gold'}>{ctx.pool.data?.status ?? 'loading'}</Badge><p className="text-sm leading-6 text-ink-700">{content[2]}</p>{section === 'security' && ctx.pool.data?.contract_id && <a href={contractExplorerUrl(ctx.pool.data.contract_id)} target="_blank" rel="noreferrer" className="block text-sm text-brand-400 hover:underline">View contract on stellar.expert ↗</a>}{section === 'archive' && <Button variant="ghost" disabled>Archive after safety checks</Button>}</Card></PoolPage>
}
...
function Loading() { return <Card><p className="text-sm text-ink-500">Reading pool data…</p></Card> }
